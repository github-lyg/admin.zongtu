...
<div class="page-content">
    <div class="page-sub-toolbar">
        <span>
        <?php if(mcv('shop.refundaddress.add')) { ?>
        	<a class="btn btn-primary btn-sm" href="<?php  echo webUrl('shop/refundaddress/add')?>">添加新退货地址</a>
        <?php  } ?>
    </span>
    </div>
    <form <?php if( mce('shop.refundaddress' ,$item) ) { ?>action="" method="post"<?php  } ?> class="form-horizontal form-validate" enctype="multipart/form-data">
        <input type="hidden" name="id" value="<?php  echo $item['id'];?>" />
        <div class="form-group">
            <label class="col-lg control-label must">退货地址名称</label>
            <div class="col-sm-9 col-xs-12">
                <?php if( mce('shop.refundaddress' ,$item) ) { ?>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-9 col-xs-12">
                ...
                <?php  } ?>
                <input type="button" name="back" onclick='history.back()' <?php if(mcv('shop.refundaddress.add|shop.refundaddress.edit')) { ?>style='margin-left:10px;'<?php  } ?> value="返回列表" class="btn btn-default" />
            </div>
        </div>
    </form>
</div>
