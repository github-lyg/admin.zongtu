...
<input type="hidden" name="merchGroup" value="<?php  echo $merchGroup['goodschecked'];?>">
<script type="text/javascript">
    ...
    function fetch_next() {
        var cates =$("#cates").val();
        var from =$("#from").val();
        var merchGroup = $("input[name='merchGroup']").val();
        var postdata =  {
            url: urls[len],
            cate: cates,
            from:from
            ...
            if (len >= total) {
                ...
                if (confirm('商品已经获取成功, 是否跳转到商品管理?')) {
                    if(merchGroup == 1)
                    {
                        ...
                        location.href = "<?php  echo merchUrl('goods' ,array('goodsfrom'=>check))?>";
